...
export const NRT: Airport = {
    ...
    routes: [
      {
        ...
        recommended: true,
        reserved: true,
        note: '객실 끝에 캐리어 두는 선반이 있어요. 닛포리에서 JR 야마노테선으로 갈아타면 도쿄 시내는 거의 다 갈 수 있어요. 라이너권 없이 타면 차내에서 300엔이 더 붙으니(합계 2,880엔) 반드시 타기 전에 사세요.',
        steps: [
          {
      ...
      },
      {
         ...
         * 이름에 「추가요금 없음」을 붙인 이유.
         *
         * 케이세이가 나리타에서 굴리는 열차가 셋인데(스카이라이너 · 액세스
         * 특급 · 본선 특급) 이름이 전부 비슷하고, 그중 둘에 「특급」이 붙어
         * 있다. 한국에서 특급은 KTX 쪽 어감이라 **「특급＝돈을 더 내는 열차」**
         * 로 읽힌다. 실제로는 일본 사철의 特急은 대개 추가요금이 없는 등급이고,
         * 돈을 더 내는 건 좌석을 지정하는 스카이라이너뿐이다.
         *
         * 게다가 스카이라이너와 이 열차는 **같은 선로**(나리타 스카이액세스선)를
         * 달려서 「액세스」라는 말까지 겹친다. 이름만으로는 구분이 안 되게
         * 생겨 먹었으니, 구분해 주는 말을 이름에 넣는다.
         */
        id: 'nrt-access',
        name: '케이세이 액세스 특급 (추가요금 없음)',
        nameJa: 'アクセス特急',
        type: 'train',
        ...
        minutes: 67,
        yen: 1372,
        note: '**스카이라이너와 다른 열차예요.** 같은 회사가 같은 선로로 굴리는데, 이쪽은 좌석 지정이 없어서 라이너권을 따로 살 필요가 없어요 — 교통카드나 표 한 장이면 돼요. 대신 통근 노선과 같은 열차라 자리 보장이 없고, 짐 두는 공간도 따로 없어요.',
        steps: [
          {
            action: '짐을 찾고 세관을 나오면 1층 도착 로비예요',
            where: '제1터미널은 남·북 윙 모두, 제2터미널도 1층이 도착층이에요',
      ...
      },
      {
        ...
        minutes: 80,
        yen: 1052,
        note: '**이것도 스카이라이너가 아니에요.** 「특급」이지만 좌석 지정이 없어서 표 한 장으로 타요. 스카이라이너보다 40분쯤 더 걸리는 대신 값이 절반 아래예요.',
      },
      {
        id: 'nrt-bus-tokyo',
        name: '에어포트 버스 도쿄·특급',
      ...
      },
      {
        ...
        ways: [
          ...
          },
          {
            ...
            label: '케이세이 본선 특급 (추가요금 없음)',
            minutes: 80,
            yen: 1052,
            transfers: 0,
            note: '⚠ 승강장이 따로예요. 개찰구를 지난 뒤 안쪽에 있는 「중간 개찰기」를 한 번 더 통과해야 본선 승강장이 나와요. 모르면 스카이라이너 승강장에서 헤매요. 40분쯤 더 걸리는 대신 1,528엔 싸요.',
          },
        ],
      },
      {
    ...
    tips: [
      ...
      '케이세이 열차가 세 가지인데 이름이 다 비슷해요. **「특급」이 붙어도 돈을 더 내는 건 스카이라이너뿐이에요.** — 스카이라이너만 좌석 지정이라 표를 두 장(운임 1,280 + 라이너권 1,300) 사고, 「액세스 특급」과 「본선 특급」은 통근 열차라 표 한 장으로 1,000엔대에 타요. 한국의 「특급＝비싼 열차」와 달라서 헷갈리기 쉬운 부분이에요.',
