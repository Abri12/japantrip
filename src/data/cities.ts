...
export interface City {
  ...
  airportIds: string[];
  /**
   * 공항 없이 **육로로** 들어가는 도시일 때, 어떻게 가는지.
   *
   * ## 왜 필요한가
   *
   * 「고를 수 있는 도시는 어떻게 가는지를 답할 수 있어야 한다」는 규칙이 있고
   * (test/coverage.test.ts), 지금까지 그 답은 언제나 공항이었다. 열려 있는
   * 도시가 전부 직항이 닿는 곳이었기 때문이다.
   *
   * 시즈오카에서 그 가정이 깨졌다. 한국인이 시즈오카에 가는 방법은 **도쿄나
   * 나고야로 날아가 신칸센을 타는 것**이지 시즈오카 공항이 아니다. 그런데
   * 규칙의 잣대가 공항 하나뿐이라, 사실대로 적으면(공항 없음) 도시를 열 수
   * 없고 열려면 없는 공항을 지어내야 했다.
   *
   * 규칙이 틀린 게 아니라 **잣대가 좁았다.** 답하는 방법을 하나 더 둔다.
   *
   * 공항이 있는 도시에는 넣지 않는다 — 두 답이 다 있으면 어느 쪽을 보여줄지
   * 매번 정해야 하고, 그건 화면마다 갈린다.
   */
  landAccess?: {
    /** 「도쿄에서 신칸센 1시간」처럼 한 줄로 */
    summary: string;
    /** 어느 도시를 거쳐 가는지 (City id) — 그 도시의 공항이 실제 관문이다 */
    via: string[];
  };
  /**
   ...
   * ## 고르는 기준
   *
   * **바로 위 `blurb` 가 말하는 것과 같은 것을 가리켜야 한다.** 카드에서 둘이
   * 나란히 보이는데 서로 다른 말을 하면, 그림이 글을 돕는 게 아니라 헷갈리게
   * 한다. 실제로 그런 것이 셋 있었다 — 다카마쓰는 「우동의 성지」인데 전골
   * 냄비(🍲)였고, 오키나와는 「아열대 휴양지」인데 고래(🐋)였고, 나고야는
   * 「도요타의 고장」인데 새우튀김(🍤)이었다.
   *
   * **겹치면 안 된다.** 도시 선택 화면은 열 장을 한 번에 보여주는 자리라,
   * 같은 그림이 둘이면 훑을 때 구분이 안 된다. `test/cities.test.ts` 가 막는다.
   *
   * **변이 선택자를 빠뜨리지 않는다.** `⛩️`·`🏝️` 처럼 뒤에 U+FE0F 가 붙어야
   * 컬러로 그려진다. 빠지면 흑백 글리프로 떨어지는데, 눈으로는 비슷해 보여서
   * 코드를 봐야 안다. 실제로 장소 화면에서 한 번 겪었다.
   */
...
export const CITIES: City[] = [
  ...
  },
  {
    ...
    blurb: '한국에서 가장 가까운 일본 도시 — 포장마차와 돈코츠라멘으로 유명해요',
    travelTip:
...

  {
    ...
    airportIds: ['hnd', 'nrt'],
    rationale:
      '노선이 세계에서 가장 복잡하고, 공항이 둘이라 도심까지 값이 세 배 넘게 차이 나요. 게다가 지하철 회사가 둘이고 그 위에 JR이 겹쳐서, 「지하철 패스」를 사도 야마노테선을 못 타요. 이 구조를 모르면 개찰구 앞에서 막히는데, 그걸 미리 알려주는 한국어 정보가 거의 없어요.',
    landmark: { emoji: '🗼', tint: '#EAF0FC' },
    blurb: '일본의 수도이자 최대 도시 — 없는 게 없는 대신, 다 보려면 나눠서 와야 해요',
    travelTip:
      '지하철(도쿄메트로·도에이)과 JR 야마노테선은 회사가 달라서 패스도 개찰구도 따로예요. 하루 동선이 어느 쪽에 몰려 있는지부터 보면 교통비가 확 줄어요.',
    contributionMultiplier: 1.0,
  },
  ...
  },
  {
    ...
    blurb: '일본 3대 도시 중 하나 — 미소카츠·히츠마부시 「나고야메시」의 고장이에요',
    travelTip:
  ...
  },
  {
    ...
    phase: 3,
    /*
     * 장소를 채우면서 coming → seeding 으로 올렸다.
     *
     * `coming` 인 도시는 **선택 목록에 아예 안 뜬다**(lib/selected-city.tsx).
     ...
     * 사람에게 실제 관문은 **도쿄나 나고야**다 — 노선이 얇아서 시즈오카
     * 공항으로 들어오는 한국인은 드물다. 확인하지 못한 노선을 데이터로
     * 넣으면 그 값이 곧 「이 공항으로 오세요」라는 안내가 되므로 비워 둔다.
     */
    airportIds: [],
    landAccess: {
      summary: '도쿄에서 신칸센 약 1시간 · 나고야에서 약 1시간',
      via: ['tokyo', 'nagoya'],
    },
