...
export const PASSES: TransitPass[] = [
  ...
  // ── 도쿄 ─────────────────────────────────────────────
  //
  // 도쿄 패스를 고를 때 실제로 걸리는 지점은 값이 아니라 **회사 경계**다.
  // 지하철은 도쿄메트로와 도에이 두 회사고, 관광지 절반은 그 위를 지나는
  // JR 야마노테선 옆에 있다. 「도쿄 지하철 패스」를 사도 야마노테선은 못 탄다.
  // 그래서 이 도시만큼은 covers 보다 excludes 를 먼저 읽어야 한다.
  //
  // 2026년 3월 14일에 아래 값이 전부 올랐다(JR동일본 운임 개정에 맞춘 조정).
  // 인터넷에 남아 있는 글은 대부분 옛날 값이라, 여기 적힌 값과 다르면
  // 그쪽이 오래된 것이다.
  {
