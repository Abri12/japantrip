...
export const OSAKA_PASSES: TransitPass[] = [
  ...
  },
  {
    ...
    excludes: [
      '사철과 JR',
      '메트로와 직통 운행하는 사철 구간 — 같은 열차를 타고 있어도 경계를 넘으면 안 돼요',
      '유메시마역',
      '관광시설 입장',
    ],
    breakEven: '지하철 네댓 번 이상 타면 이득',
    worthIt:
      '관광시설엔 관심 없고 지하철만 많이 탈 때 좋아요. 한 번에 190~290엔이라(구간이 멀수록 비싸요) 네댓 번은 타야 820엔을 넘겨요. 난바~우메다처럼 미도스지선 안에서만 오가면 190~240엔짜리가 이어져서 다섯 번은 타야 하고, 주말이면 620엔이라 세 번이면 넘어가요.',
    ...
    caution:
      '주말이 200엔 더 싸요. 일정을 조정할 수 있다면 지하철 많이 타는 날을 주말로 몰면 이득이에요. 사철 직통 구간은 갈아타지 않아도 요금이 새로 붙으니, 내릴 역이 메트로 구간인지 보고 타세요.',
    checkedAt: '2026-08',
    // 오사카메트로 공식 안내로 확인 (2026-08): 평일 820 · 토일공휴일 620,
    // 메트로 전선(유메시마 제외) · 뉴트램 · 오사카 시티버스.
    verified: true,
  },
];
...
export const OSAKA_ADVISORY: PassAdvisory = {
    ...
    body: '주유패스(3,500엔)의 값어치는 **지하철이 아니라 관광시설 무료 입장**에 있어요. 오사카성 천수각이 1,200엔으로 오르면서, 천수각과 공중정원 두 곳만 들어가도 지하철값까지 치면 본전이 넘어요. 지하철만 탈 거면 에코카드(평일 820엔)가 훨씬 나아요. 다만 시설마다 시간 조건이 붙는 게 있으니 그날 동선부터 보세요.',
  };
