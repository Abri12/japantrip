 ...
 * ── 문장 안의 강조 ──────────────────────────────────
 *
 * 설명글에 `**이렇게**` 를 쓰면 화면이 그 부분만 굵게 그린다(`components/ui/text.tsx`).
 * 규칙은 하나다 — **한 문단에 한 군데.** 이 앱의 글은 「왜 그런지」까지 적느라
 * 길어서, 강조가 두 개 넘어가면 굵은 글씨끼리 경쟁하다가 아무것도 안 읽힌다.
 *
 * 고르는 기준은 「중요한 말」이 아니라 **판단을 가르는 구절**이다. 「난카이는
 * 텐노지에 안 가요」는 그걸 모르면 잘못 내리니까 굵고, 「짐 두는 선반이 있어요」는
 * 알면 좋지만 선택이 달라지지 않으니까 안 굵다.
 */
