 ...
 * ## 범용 결제수단으로 바꿔 주지 않는다
 *
 * 네이버페이 포인트 전환이 여기 있었는데 뺐다. 나머지는 **특정 상품**과
 * 바꾸는 것이지만 그건 아무 데나 쓸 수 있는 돈으로 바꿔 주는 것이라, 성격이
 * 다른 항목이었다.
 *
 * 두 군데가 동시에 나빠진다. 환금성이 생기면 규제 판단이 나빠지고 — 크레딧이
 * 「사실상 현금」에 가까워질수록 선불전자지급수단 판단이 무거워진다 — 동시에
 * **담합의 수익이 확실해진다.** 받는 것이 아무 데나 쓸 수 있는 돈이면 가짜
 * 제보의 기대수익이 최대가 되기 때문이다. 특정 상품이면 「그게 필요한
 * 사람에게만」 가치가 있어서 그만큼 못 미친다.
 *
 * **다시 넣지 않는다.** 넣으려면 docs/CREDITS.md 의 근거부터 다시 봐야 한다.
 *
 * ## 나머지는 전부 `partner` 다
 * 즉 지금 이 목록대로 켜면 크레딧은 선불전자지급
