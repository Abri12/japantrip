...
describe('실제 데이터와 맞물린다', () => {
  ...
  it('live 라고 적은 도시는 실제로 채워져 있다', () => {
    /*
     * 이 파일 머리말에 적힌 사고가 **상태 값에서도 똑같이 난다.**
     * 나고야가 「데이터 모으는 중」인데 장소가 0개였던 것처럼, `live` 라고
     * 적어 두고 속이 비어 있으면 사용자는 정식으로 채워진 도시인 줄 알고
     * 고른다. 손으로 적는 값이라 데이터가 늘어도 저절로 안 따라온다.
     *
     * `live` 의 뜻이 「큐레이션 데이터가 채워져 있다」이므로,
     * 커버리지가 rich(장소 12곳 이상)인 것과 **같은 말이어야 한다.**
     * 패스도 최소 하나는 있어야 「이동」 화면이 빈손이 아니다.
     */
    for (const city of CITIES.filter((c) => c.status === 'live')) {
      const c = cityCoverage(city.id, city.airportIds);
      strictEqual(c.level, 'rich', `${city.name} 은 live 인데 ${c.level} 이에요`);
      ok(c.passes > 0, `${city.name} 은 live 인데 교통패스가 없어요`);
    }
  });
